...
number_of_rows <-
  df_data %>%
    filter(!is.na(who_wanted_breakup_more_nonmar)) %>%
    summarize(N = n())
...
males_want_break_up / number_of_rows
females_want_break_up / number_of_rows

```

```{r}

convert_political <- function(x) {
  case_when(
    x == 1 ~ "Strong Republican",
    x == 2 ~ "Not Strong Republican",
    x == 3 ~ "Leans Republican",
    x == 4 ~ "Undecided/Independent/Other",
    x == 5 ~ "Leans Democrat",
    x == 6 ~ "Not Strong Democrat",
    x == 7 ~ "Strong Democrat"
  )
}
...
df_data %>%
    ...
    mutate(partyid7 = convert_political(partyid7)) %>%
  pull(partyid7)
...
number_match <- 
  df_data %>%
    filter(!is.na(partyid7) | !is.na(partner_polictial_belief)) %>%
    mutate(partyid7 = convert_political(partyid7), 
           partner_polictial_belief = convert_political(partyid7),
           political_diff = abs(partyid7 - partner_polictial_belief)) %>%
    filter((political_diff < 2)) %>%
    summarize(N = n())
